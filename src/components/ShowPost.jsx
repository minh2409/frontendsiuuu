...
function ShowPost(props) {
  ...
  const submitHandler = async (e) => {
    ...
    try {
      ...
      await axiosJWT.post("https://ffilm.onrender.com/api/comment/", submitData, {
  ...
  useEffect(() => {
    const fetchComment = async () => {
      const res = await axios.get(
        `https://ffilm.onrender.com/api/comment/${props.post._id}`
      );

      res.data.comments.forEach(async (comment) => {
        const res = await axios.get(
          `https://ffilm.onrender.com/api/user/${comment.user}`
        );
